...
getUserList(classname,currentPage,avageNum);
initPagination(totalNum,totalPage);

//与后台交互获取数据，异步加载到页面上
function getUserList(classname,pageNum,pageSize){
    currentPage = pageNum;
    var phone = $("#phone").val();
    ...
    $.ajax({
        ...
        data:{"phone":phone,"classname":classname, "pageNum":pageNum,"pageSize":pageSize},
        dataType : 'json',
...
function initPagination(totalPage,totalNum) {
    ...
    for (var i = 1; i <= totalPage; i++) {
        $("#pagination ul").append(
            '<li id="page'+i+'"><a href="javascript:void(0);"  onclick="getUserList(classname,' + i + ',' + avageNum + ')">' + i + '</a>'
        )
    }
    $("#pagination ul").append(
...
$("#pagination").on("click","li",function(){
    ...
    if (aText == "上一页"){
        ...
        getUserList(classname,currentPage - 1,avageNum);
        checkA();
    }else if(aText == "下一页"){
        $(".pagination > li").removeClass("active");
        $("#page"+(currentPage + 1)).addClass("active");
